...
describe('Liking A Restaurant', () => {
  ...
  it('should show the like button when the restaurant has not benn liked before', async () => {
    await TestFactories.createLikeButtonPresenterWithRestaurant({ id: 1 });

    expect(document.querySelector('[aria-label="like this restaurant"]')).toBeTruthy();
  });

  it('should now show the unlike button when restaurant has not been liked before', async () => {
    await TestFactories.createLikeButtonPresenterWithRestaurant({ id: 1 });
  ...
  it('should be able to like the restaurant', async () => {
    ...
    const db = await FavoriteRestaurantIdb.getRestaurant(1);
